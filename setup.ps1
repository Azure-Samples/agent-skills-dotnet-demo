...
Write-Host "Prerequisites OK (azd, dotnet)" -ForegroundColor Green
...
if (-not $endpoint) {
    ...
    # Fallback: query Azure directly for cognitive services accounts in the azd resource group
...
if ($endpoint) {
    ...
    Write-Host "  dotnet user-secrets set --id $secretsId `"AzureOpenAI:Deployment`" `"gpt-5-mini`""
}
